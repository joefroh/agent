...
class OllamaLLM:
    system_prompt = create_message('system',"/no_think You are a helpful assistant who keeps things brief and responds only in english. Not every prompt requires a tool, if you don't think you need to use one - just respond normally as if you didn't have any provided. When in doubt, do the simplest thing, and keep responses concise.")
    ...
    def __send_messages_to_llm(self, options=default_options):
        if self.support_tools:
            try:
                return self.client.chat(model=self.model_name, messages=self.messages, options=options, tools=self.tools)
            except _types.ResponseError as e:
                if e.error.__contains__("does not support tools"):
                    self.support_tools = False # Set tool support to false and fall through
                else: # Got some other type of error we haven't handled yet - spit it out and kill the process
                    print(e.error)
                    exit(-1)

        return self.client.chat(model=self.model_name, messages=self.messages, options=options)